...
def main(chara: character, result):
    #     import cProfile
    #     cProfile.run('''codes''')
    #     exit()
...
def printMonthly(chara: character):
    ...
    main(chara, result)
    result = np.array(result)  # , dtype=np.dtype([('damage', np.float_), ('bigdamage', np.float_)]))
    # print(result)
    print(result.shape)
    ...
    for i in range(MONTH):
        # 中位数
        rslt = result[i]
        # rslt = np.sort(rslt, order='damage')
        rslt = np.sort(rslt, 1)
        # print(rslt)
        # print(rslt[PERSONS//2])
        middamage, midbigdamage = rslt[PERSONS // 2]
        # print(rslt[..., 0])
        damages = rslt[..., 0]
        if DEBUG:
            print(f'{len(damages) = }')
...
def midNums(lis: np.ndarray, len):
